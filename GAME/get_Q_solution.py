...
def main():
    ...
    N = 50 #number of Decision Epochs (for 1/5 box size it should get a bit larger N because more steps are needed to get to the end)
    n_episodes = 500 
    ...
    make_figures(n_episodes, distances_per_episode, loss_per_episode, total_reward_per_episode, epsilon_per_episode)


def make_figures(n_episodes, distances_per_episode, loss_per_episode, total_reward_per_episode, epsilon_per_episode):
...
def  Q_table_to_csv(Q_table_per_episode, N):
    ...
    for Q_table in Q_table_per_episode:
        state, info = env.reset(start_point = env.get_middle_box_center())
        action_list = []
        for i in range(N):
            #print(Q_table[state])
